...
function _formData() {
    return `<div class="row m-2 shadow">`
                +_formIcon({
                    ...
                    ,text:"<h3>Daftar Indikator Kinerja Daerah</h3>",
                    classJudul:' p-2',
                    id:"form1",
                    btn:_btn({
...
function addDataed(){
    ...
    _postFile('proses/insIkd',param,img.data).then(res=>{
        res=JSON.parse(res);
        if(res.exec){
            img.data=[];
            _modalHide('modal');
...
function updDataed(ind){
    ...
    _post('proses/updIkd',param).then(res=>{
        res=JSON.parse(res);
        if(res.exec){
            _modalHide('modal');
            _respon(res.data);
    ...
    _post('proses/execQueryNoReturn',{
        query:(
                ...
                `INSERT INTO ikuvalue(kdIku, tahun,target, realisasi, capaian,jenis) VALUES (
                    '`+param.kdIku+`','`+param.tahun+`','`+param.target+`','`+param.realisasi+`','`+param.capaian+`','ikd'
                )`
                :
                `
                    update ikuvalue set target='`+param.target+`',realisasi='`+param.realisasi+`',capaian='`+param.capaian+`'
                    where kdIku='`+param.kdIku+`' and tahun='`+param.tahun+`' and jenis='ikd'
                `
            )
    }).then(res=>{
        res=JSON.parse(res);
