...
function addDataed(){
    ...
    _postFile('proses/insIku',param,img.data).then(res=>{
        res=JSON.parse(res);
        if(res.exec){
            img.data=[];
            _modalHide('modal');
...
function updDataed(ind){
    ...
    _post('proses/updIku',param).then(res=>{
        res=JSON.parse(res);
        if(res.exec){
            _modalHide('modal');
            _respon(res.data);
    ...
    _post('proses/execQueryNoReturn',{
        query:(
                ...
                `INSERT INTO ikuvalue(kdIku, tahun,target, realisasi, capaian,jenis) VALUES (
                    '`+param.kdIku+`','`+param.tahun+`','`+param.target+`','`+param.realisasi+`','`+param.capaian+`','iku'
                )`
                :
                `
                    update ikuvalue set target='`+param.target+`',realisasi='`+param.realisasi+`',capaian='`+param.capaian+`'
                    where kdIku='`+param.kdIku+`' and tahun='`+param.tahun+`' and jenis='iku'
                `
            )
    }).then(res=>{
        res=JSON.parse(res);
